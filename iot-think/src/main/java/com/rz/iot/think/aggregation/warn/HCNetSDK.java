...
/**
 *
 * @author Xubinfeng  update by qinxian
 */
...
public interface HCNetSDK extends StdCallLibrary {
    ...
    String path3 = path2.replace("target/classes/","");
    String path = path3 + "\\src\\main\\resources\\lib\\aggregation\\alarm\\HCNetSDK";
...
//校时结构参数
	...
	public NET_DVR_TIME(int dwYear, int dwMonth, int dwDay, int dwHour, int dwMinute, int dwSecond) {
		super();
		this.dwYear = dwYear;
...
public static class NET_DVR_DISKSTATE extends Structure {//硬盘状态
	...
	@Override
	public String toString() {
		return "NET_DVR_DISKSTATE [dwVolume=" + dwVolume + ", dwFreeSpace=" + dwFreeSpace + ", dwHardDiskStatic="
				+ dwHardDiskStatic + "]";
	}
...
public static class NET_DVR_WORKSTATE_V30 extends Structure {//DVR工作状态(9000扩展)
	...
	public int  dwLocalDisplay;//本地显示状态,0-正常,1-不正常
		...
		@Override
		public String toString() {
			return "NET_DVR_WORKSTATE_V30 [dwDeviceStatic=" + dwDeviceStatic + ", struHardDiskStatic="
					+ Arrays.toString(struHardDiskStatic) + ", struChanStatic=" + Arrays.toString(struChanStatic)
					+ ", byAlarmInStatic=" + Arrays.toString(byAlarmInStatic) + ", byAlarmOutStatic="
					+ Arrays.toString(byAlarmOutStatic) + ", dwLocalDisplay=" + dwLocalDisplay + ", byAudioChanStatus="
					+ Arrays.toString(byAudioChanStatus) + ", byRes=" + Arrays.toString(byRes) + "]";
		}
...
public static class NET_DVR_WORKSTATE_V40 extends Structure {//DVR工作状态(新增)
	...
	public int  dwLocalDisplay;//本地显示状态,0-正常,1-不正常	
    public  byte [] byAudioChanStatus = new byte[MAX_AUDIO_V30];//表示语音通道的状态 0-未使用，1-使用中, 0xff无效
    public  byte[]  byRes1 = new byte[10];
    
    public float fHumidity;//传感器获知的湿度，取值范围：0.0~100.0 
    public float fTemperature;//传感器获知的温度，取值范围：-20.0~90.0 
...
//报警主机日志查询结构体
public static class NET_DVR_ALARMHOST_SEARCH_LOG_PARAM extends Structure {
	public short wMajorType;//主类型 1-报警;2-异常;3-操作;4-事件
	public short wMinorType;//次类型 1-报警;
	public NET_DVR_TIME struStartTime;//开始时间
	public NET_DVR_TIME struEndTime;//结束时间 
	public byte[] byRes = new byte[8];
	@Override
	public String toString() {
		return "NET_DVR_ALARMHOST_SEARCH_LOG_PARAM [wMajorType=" + wMajorType + ", wMinorType=" + wMinorType
				+ ", struStartTime=" + struStartTime + ", struEndTime=" + struEndTime + ", byRes="
				+ Arrays.toString(byRes) + "]";
	}
...
//报警主机日志查找结构体
public static class NET_DVR_ALARMHOST_LOG_RET extends Structure {
...
public static class NET_DVR_ALARMHOST_MAIN_STATUS_V40 extends Structure {
...
//前面板锁定
 ...
 //报警主机 日志查询(返回句柄)
 NativeLong NET_DVR_FindAlarmHostLog(NativeLong lUserID, int lSelectMode, NET_DVR_ALARMHOST_SEARCH_LOG_PARAM lpSearchParam);
 //报警主机 逐条获取日志信息
 NativeLong NET_DVR_FindNextAlarmHostLog(NativeLong lFindHandle, NET_DVR_ALARMHOST_LOG_RET lpFindData);
 //关闭报警主机日志查找,释放资源。
 boolean NET_DVR_FindAlarmHostLogClose(NativeLong lFindHandle);
 //辅助功能控制
 boolean NET_DVR_AlarmHostAssistantControl(NativeLong lUserID,short dwType,short dwNumber,short dwCmdParam);
...
{
    PlayCtrl INSTANCE = (PlayCtrl) Native.loadLibrary("lib\\aggregation\\PlayCtrl",
            PlayCtrl.class);
...
{
    GDI32 INSTANCE = (GDI32) Native.loadLibrary("lib\\gdi32", GDI32.class, DEFAULT_OPTIONS);
...
/*interface USER32 extends W32API
{
    ...
    USER32 INSTANCE = (USER32) Native.loadLibrary("lib\\user32", USER32.class, DEFAULT_OPTIONS);
