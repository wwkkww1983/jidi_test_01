...
public class IotScreenProgramServiceImpl implements IotScreenProgramService {
    ...
    public Result update(IotScreenProgramAddParam param) {
        ...
        IotScreenProgramInfo programInfo = iotScreenProgramInfoMapper.selectByPrimaryKey(programId);
        if (programInfo.getVerify() != RzIotDBConstParam.MATERIAL_VERIFY_STATUS_OF_NOT_VERIFY) {
            // 节目已经被审核，不允许更新
            result.setCode(RzIotErrMessage.UPDATE_SCREEN_PROGRAM_HAVING_BEEN_VERIFIED);
            return result;
        }
