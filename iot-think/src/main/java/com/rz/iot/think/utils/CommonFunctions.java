...
public class CommonFunctions {
    ...
    public static String getIpAddress(HttpServletRequest request) {
        ...
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = request.getHeader("Proxy-MqttTest-IP");
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
            ip = request.getHeader("WL-Proxy-MqttTest-IP");
        }
        if (ip == null || ip.length() == 0 || "unknown".equalsIgnoreCase(ip)) {
     ...
     * @return
     */
    public static FileShowSimple fileUpload(MultipartFile file){
        try{
            if (!file.isEmpty()) {
                FileShowSimple fileShowSimple = new FileShowSimple();
                //获得原始文件名
                ...
                String path = fastDFSUtils.uploadFile(baos.toByteArray(), extName);
                String fdfsNginxServer = PropertiesUtil.getProperty("FastDFS.properties", "fdfs_nginx_server");
                fileShowSimple.setUrl(fdfsNginxServer + "/" + path);
                fileShowSimple.setName(originalFilename);
                fileShowSimple.setSize(file.getSize());
                return fileShowSimple;
            }
        } catch (IOException e) {
            log.error(e.getMessage(), e);
        } catch (MyException e) {
